  ...
  const self = observable({
    ...
    get completedItems() {
      if (self.seachFilter) {
        return self.items.filter(
          (item) =>
            item.tags.indexOf(this.seachFilter) !== -1 && item.isComplete
        );
      } else return self.items.filter((i) => i.isComplete);
    },
    ...
    get inProgressItems() {
      if (self.seachFilter) {
        return self.items.filter(
          (item) =>
            item.tags.indexOf(this.seachFilter) !== -1 && item.isInProgress
        );
      } else return self.items.filter((i) => i.isInProgress);
    },
    get getUniquetags() {
      let tags = [];
